 ...
 * <em>indices</em> with the <code>CHAVEDocument</code>s...
 * 
 * @author   Ricardo Rodrigues
 * @version  0.9.9.9
 *
 */
public class CLEFCHAVEIndicesCreatorProgram {
  public static void main(String[] args) {
    LocalDateTime startTime = LocalDateTime.now();
    System.out.println("CLEFCHAVEIndicesCreatorProgram");
    System.out.println();
    System.out.println("Start Time: " + startTime);
    System.out.println();

    String chaveCorpusDirectory = null;
    String documentIndexDirectory = null;
    String sentenceIndexDirectory = null;
    String factIndexDirectory = null;

    CHAVEDocumentIndex documentIndex = null;
    ...
    String usage = "java program.CLEFCHAVEIndicesCreatorProgram "
        + "-c chaveDataDirectory\n-d documentIndexDirectory "
        + "-s sentenceIndexDirectory -f factIndexDirectory\n\n"
        + "You must specifiy the location of the SGML data files, that are "
        + "used to populate\nthe indices, and also the location where those "
        + "indices should be created (or appended).\n\n"
        + "The program's options are:\n"
        + "-c: the location of the CHAVE SGML data files (if a directory,"
        + " it will be\n\trecursively listed)\n"
    ...
    if ((chaveCorpusDirectory == null) || (documentIndexDirectory == null)
    ...
    try {
      documentIndex = new CHAVEDocumentIndex(FSDirectory.open(
    ...
    File[] editions = null;
    try {
      editions = GeneralTools.listFiles(new File(chaveCorpusDirectory), "sgml");
    }
    catch (FileNotFoundException fnfe) {
      fnfe.printStackTrace();
    }

    System.out.println("Processing files (editions)...");
    if (editions.length > 0) {
      for (File edition : editions) {
        System.out.println("input: " + edition.getAbsolutePath());
        CHAVEDocument[] documents = null;
        try {
          documents = XMLTools.readCHAVEEdition(edition, "UTF-8");
        }
        catch (ParserConfigurationException pce) {
          pce.printStackTrace();
        }
        ...
        for (CHAVEDocument document : documents) {
          System.out.println("document: " + document.getID());
          boolean sentencesWithFactsFound = false;
          String[] sentences = splitter.split(document.getText(), true);
          ...
          if (sentencesWithFactsFound) {
            try {
              documentIndex.add(new CHAVEDocument(document.getNumber(),
                  document.getID(), document.getDate(),
                  document.getCategory(), document.getAuthor(),
                  document.getText()));
            }
            catch (IOException ioe) {
              ioe.printStackTrace();
            ...
            System.out.println("[no facts found in document: "
                + document.getID() + "]");
          }
        }
      }
